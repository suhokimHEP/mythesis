...
Figure \ref{fig:Trigger Efficiency} and \ref{fig:Trigger Efficiency2} are the trigger efficiency of various BPH trigger paths for different mass scale and lifetime points of the signal ($H \to SS \to \tau^{+}\tau^{-}\tau^{+}\tau^{-}$) sample
The signal process shows an overall good efficiency.
Signal points with LLP's c$\uptau$ = 10mm and c$\uptau$ = 100mm show the best performance.
Signal points with LLP's c$\uptau$ = 1000mm likely decay outside of the tracker region, leaving no track's impact parameter, and fails to pass the trigger.
On the other hand signal point with LLP's c$\uptau$ = 1mm may not reach the first-pixel detector, which is at 2.7cm from the beam spot, and fails to pass the trigger.
...
\begin{figure}[h!]
  ...
  \includegraphics[width=0.47\linewidth]{figs/TrigEff_HLT_Mu8_IP3_part.pdf}
	\caption{The plots show the trigger efficiency for each HLT path with respect to LLP's lifetime. Each line denotes mass scale of each LLP. Please note the efficiency is set to 0 for MS=7\GeV c$\uptau$ = 1mm due to absece of Monte Carlo (MC).}
